				...
				myvali : function(id) {
					...
					var valiform = {
						...
						vphone : function() {
							...
							if ($(this).val() == "") {
								$(this).siblings("p").html("璇疯緭鍏ユ墜鏈哄彿")
										.removeAttr("class").css({
											"color" : errCol[0]
										})
							} else if (!strphone.test($(this).val())) {
								$(this).siblings("p")
										.html("璇疯緭鍏�11浣嶆纭殑鎵嬫満鍙�").removeAttr(
												"class").css({
											"color" : errCol[0]
										});
								if (isPhoneCode[0] == 1) {
									if (isMyCode[0] == 1) {
										if (myCode.siblings("p").hasClass(
												"correct")
												&& $(this).siblings("p")
														.hasClass("correct")) {
											valiform.ismsgbtn.apply(myphone)
										} else {
											myform.find(phoneCodeBtn).css(
													"color", codeBtnCol1[0])
													.attr("disabled",
															"disabled")
										}
									} else {
										if ($(this).siblings("p").hasClass(
												"correct")) {
											valiform.ismsgbtn.apply(myphone)
										} else {
											myform.find(phoneCodeBtn).css(
						...
						msg : function() {
							...
							if (b == true) {
								...
								myform.find(phoneCodeBtn).attr("disabled",
										"true").css("color", codeBtnCol1[0])
							}
							function yz() {
								a--;
								if (a < 0) {
									...
									$this.val("閲嶆柊鑾峰彇");
									b = true;
									myform.find(phoneCodeBtn).removeAttr(
											"disabled").css("color",
											codeBtnCol2[0]);
									clearTimeout(ti)
								} else {
						...
						yzm : function() {
							...
							if (isPhoneCode[0] == 1) {
								...
								myform.find(phoneCodeInput).siblings("p").html(
										"鈭�").attr({
									"class" : "correct"
								}).css({
									"color" : corrCol[0]
						...
						name1 : function() {
							...
							if (Tsval == "") {
								$(this).siblings("p").removeAttr("class").css({
									"color" : errCol[0]
								}).html("璇疯緭鍏ョ敤鎴峰悕")
							} else if (Tsval.length < NameMinLength[0]
									|| Tsval.length > NameMaxLength[0]) {
								$(this).siblings("p").removeAttr("class").css({
									"color" : errCol[0]
								}).html(
										"璇疯緭鍏�" + NameMinLength[0] + "鑷�"
												+ NameMaxLength[0]
												+ "浣嶅瓧绗︾殑鐢ㄦ埛鍚�")
							} else {
								if (nameIsserver[0] == 1) {
						...
						password : function() {
							...
							} else if ($(this).val().length < PasswordMinLength[0]
									|| $(this).val().length > PasswordMaxLength[0]) {
								$(this).siblings("p").removeAttr("class").css({
									"color" : errCol[0]
								}).html(
										"璇疯緭鍏�" + PasswordMinLength[0] + "鑷�"
												+ PasswordMaxLength[0]
												+ "浣嶅瓧绗︾殑瀵嗙爜")
							} else {
								if (PwdStrong[0] == 1) {
									if (Num.test($(this).val())
									...
									} else if (NumEl.test($(this).val())
											|| NumEl1.test($(this).val())
											|| NumEl2.test($(this).val())) {
										$(this).siblings("p").attr({
											"class" : "correct"
										}).css({
											"color" : corrCol[0]
										...
										$(this).siblings("p").removeAttr(
												"class").css({
											"color" : errCol[0]
										}).html("璇疯緭鍏ユ暟瀛楀瓧姣峗.缁勬垚鐨勫瘑鐮�")
									}
								} else {
									$(this).siblings("p").attr({
										"class" : "correct"
							...
							if ($(this).val() != mypasswords.val()
									&& mypasswords.val() != "") {
								mypasswords.siblings("p").removeAttr("class")
										.css({
											"color" : errCol[0]
										}).html("涓ゆ杈撳叆鐨勫瘑鐮佷笉涓€鑷�")
							} else if (mypasswords.val() == "") {
								mypasswords.siblings("p").html("")
							} else {
								mypasswords.siblings("p").attr({
									"class" : "correct"
								}).css({
									"color" : corrCol[0]
								}).html("鈭�")
						...
						name2 : function() {
							...
							} else if (Tsval.length < NameMinLength2[0]
									|| Tsval.length > NameMaxLength2[0]) {
								$(this).siblings("p").html(
										"璇疯緭鍏�" + NameMinLength2[0] + "鑷�"
												+ NameMaxLength2[0]
												+ "浣嶅瓧绗︾殑鏄电О").removeAttr(
										"class").css({
									"color" : errCol[0]
								})
							} else {
							...
							if (Tsval == "") {
								$(this).siblings("p").removeAttr("class").css({
									"color" : errCol[0]
										...
										RequiredsTps[reqtps.index($(this)
												.parent())])
							} else if (Reqlength.length != 0) {
								if (Tsval.length >= Reqlength[reqtps.index($(
										this).parent())][0]
										&& Tsval.length <= Reqlength[reqtps
												.index($(this).parent())][1]) {
									$(this).siblings("p").html("鈭�").attr({
										"class" : "correct"
									}).css({
								...
								} else if (Tsval.length < Reqlength[reqtps
										.index($(this).parent())][0]
										|| Tsval.length > Reqlength[reqtps
												.index($(this).parent())][1]) {
									$(this)
						...
						codes : function() {
							var $this = $(this), Tsval = $this.val().replace(
									/\s/g, "");
							if (Tsval == "") {
								$(this).siblings("p").removeAttr("class").css({
									"color" : errCol[0]
						...
						vform : function() {
							if (myform.find(Required).length >= 1) {
								for (var i = 0; i < myform.find(Required).length; i++) {
									if (myform.find(Required).eq(i).val() == "") {
										myform.find(Required).eq(i).siblings(
												"p").removeAttr("class").css({
											"color" : errCol[0]
										}).html(RequiredTps[0])
									}
								}
							...
							if (myform.find(Requireds).length >= 1) {
								for (var j = 0; j < myform.find(Requireds).length; j++) {
									if (myform.find(Requireds).eq(j).val() == "") {
										myform.find(Requireds).eq(j).siblings(
												"p").removeAttr("class").css({
											"color" : errCol[0]
										}).html(RequiredsTps[j])
									}
							...
							if (myname1.val() == "") {
								myname1.siblings("p").removeAttr("class").css({
									"color" : errCol[0]
								}).html("璇疯緭鍏ョ敤鎴峰悕")
							...
							if (myname2.val() == "") {
								myname2.siblings("p").removeAttr("class").css({
									"color" : errCol[0]
								}).html("璇疯緭鍏ユ樀绉�")
							...
							if (myphone.val() == "") {
								myphone.siblings("p").removeAttr("class").css({
									"color" : errCol[0]
								}).html("璇疯緭鍏ユ墜鏈哄彿")
							...
							if (myphone1.val() == "") {
								myphone1.siblings("p").removeAttr("class").css(
										{
											"color" : errCol[0]
										}).html("璇疯緭鍏ユ墜鏈哄彿")
							...
							if (isPhoneCode[0] == 1) {
								if (phoneCodeInput.val() == "") {
									phoneCodeInput.siblings("p").removeAttr(
											"class").css({
										"color" : errCol[0]
									}).html("璇疯緭鍏ラ獙璇佺爜")
							...
							if (mymailbox.val() == "") {
								mymailbox.siblings("p").removeAttr("class")
										.css({
											"color" : errCol[0]
										}).html("璇疯緭鍏ラ偖绠卞彿")
							...
							if (myCard.val() == "") {
								myCard.siblings("p").removeAttr("class").css({
									"color" : errCol[0]
								}).html("璇疯緭鍏ヨ韩浠借瘉")
							...
							if (mypassword.val() == "") {
								mypassword.siblings("p").removeAttr("class")
										.css({
											"color" : errCol[0]
										}).html("璇疯緭鍏ュ瘑鐮�")
							...
							if (myCode.val() == "") {
								myCode.siblings("p").removeAttr("class").css({
									"color" : errCol[0]
								}).html("璇疯緭鍏ラ獙璇佺爜")
					...
					if (isPhoneCode[0] == 1) {
						myform.find(phoneCodeBtn).css("color", codeBtnCol1[0])
								.attr("disabled", "true")
					} else {
										...
										myform
												.submit(function() {
													valiform.vform
															.apply(myform);
													var cunt = 0;
													for (var i = 0; i < myform
															.find(myVali).length; i++) {
														if (myform.find(myVali)
																.eq(i)
																.children("p")
																.attr("class") == "correct"
																|| myform
